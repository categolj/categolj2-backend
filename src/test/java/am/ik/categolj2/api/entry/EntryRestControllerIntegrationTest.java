...
public class EntryRestControllerIntegrationTest {
    ...
    @Test
    public void testCreateAsciidocEntryInAdmin() throws Exception {
        String user = "admin";
        String accessToken = getAccessToken(user, "demo");
        Entry target = new Entry(null, "Support Asciidoc!", "[lowerroman, start=5]\n" +
                ". Five\n" +
                ". Six\n" +
                "[loweralpha]\n" +
                ".. a\n" +
                ".. b\n" +
                ".. c\n" +
                ". Seven", "asciidoc", Categories.fromCategory("aa::bb::cc").getCategories(), true, Arrays.asList());
        target.setCreatedBy(user);
        target.setLastModifiedBy(user);
        EntryResource input = new EntryResource(null,
                null,
